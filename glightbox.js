(function (global, factory) {
	...
	function slideVideo(slide, data, index, callback) {
		...
		// slideMedia.style.margin = '5rem';
		slideMedia.style.backgroundColor = 'rgba(0, 0, 0, 0.35)';
		injectAssets(this.settings.plyr.js, 'Plyr', function () {
			var srcElement = _this.elements[_this.index].node;
			...
			if (provider === 'local' || !provider) {
				...
				html += 'style="background:black; max-width: '.concat(data.width, ';" ');
				html += 'preload="metadata" ';
				html += 'x-webkit-airplay="allow" ';
				html += 'playsinline ';
				html += srcElement.hasAttribute('controls') ? 'controls ' : '';
				html += srcElement.hasAttribute('loop') ? 'loop ' : '';
				html += srcElement.hasAttribute('autoplay') ? 'autoplay ' : '';
				html += srcElement.hasAttribute('muted') ? 'muted ' : '';
			...
			if (!hasAudio(srcElement)) {
				var index = playerConfig.controls.indexOf('mute');
				if (index > -1) {
					playerConfig.controls.splice(index, 1);
				}

				index = playerConfig.controls.indexOf('volume');
				if (index > -1) {
					playerConfig.controls.splice(index, 1);
				}
			}
			...
			player.on('ready', function (event) {
				...
				event.target.setAttribute('data-fullscreenScrollPosition', window.scrollY);
			...
			player.once('timeupdate', function (event) {
				player.elements.container.style.visibility = 'visible';
			});

			player.once('canplay', function (event) {
				player.forward(_this.elements[_this.index].node.currentTime);
			});
	...
	function handleMediaFullScreen(event) {
		...
		if (event.type === 'exitfullscreen') {
			...
			window.scrollTo({
				top: event.target.dataset.fullscreenscrollposition,
				behavior: 'instant'
			});
		}
	}
